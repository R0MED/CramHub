...
from sqlalchemy import or_  # Импортируем or_ для поиска по нескольким полям
...
def get_public_collections(search: str | None = None, db: Session = Depends(get_db)):
    """
    Получение всех публичных коллекций с возможностью поиска.
    Поиск ищет совпадения в названии (title) ИЛИ описании (description).
    """
    query = db.query(Collection).filter(Collection.is_public == True)
    
    if search:
        # Ищем в title ИЛИ description, игнорируя регистр (ilike)
        query = query.filter(
            or_(
...
def get_collections(search: str | None = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
    Получение коллекций текущего пользователя с возможностью поиска.
    """
