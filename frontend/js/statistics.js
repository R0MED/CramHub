...
async function loadStatistics() {
    ...
    try {
        ...
        // 1. Получаем сводку
        const sumRes = await fetch("http://127.0.0.1:8000/cards/statistics/summary", { headers });
        if (!sumRes.ok) {
        ...
        const actRes = await fetch("http://127.0.0.1:8000/cards/statistics/activity", { headers });
        if (!actRes.ok) {
